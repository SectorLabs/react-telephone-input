...
var ReactTelephoneInput = exports.ReactTelephoneInput = (_temp = _class = function (_Component) {
  ...
  this.handleFlagItemClick = function (country) {
    ...
    if (currentSelectedCountry.iso2 !== nextSelectedCountry.iso2) {
      ...
      var exampleNumber = phoneUtil.getExampleNumberForType(nextSelectedCountry.iso2, 1);
      var exampleNumberValue = '';
      if (exampleNumber) {
        exampleNumberValue = exampleNumber.getNationalNumber();
        exampleNumberValue.replace;
      }

      // If there is no input from user
      if (newNumber === nextSelectedCountry.dialCode) {
        // Add country based placeholder first mobile digit
        newNumber += exampleNumberValue[0];
        // Replace all other digits with x
        newNumber += exampleNumberValue.substring(1).replace(/\d/g, 'x');
      }
